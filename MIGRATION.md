...
- Packages `@hv/uikit-common-icons` and `@hv/uikit-common-utils` are now deprecated, are will no longer receive updates. You can still use the other releases if you need them. As for their contents:
  ...
  - Package `@hv/uikit-common-icons` was integrated in `@hv/uikit-react-icons` package. SVG files can be found in `assets/` folder.
  - Package `@hv/uikit-common-utils` was integrated in `@hv/uikit-react-core` package, under `/dist/utils`.
...
To import an icon you should use:

```js
import { <IconName> } from "@hv/uikit-react-icons/dist";
```
