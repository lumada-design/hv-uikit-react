...
- **pentaho** — aligned with the Pentaho Design System

Use the `HvProvider` to configure the active theme and color mode:

```tsx
<HvProvider theme={pentaho} colorMode="light">
  <App />
</HvProvider>
```
...
<HvProvider theme={myTheme} colorMode="sand">
  <App />
</HvProvider>;
```
...
const {
  ...
  changeMode, // Function to switch color mode
...
const DefaultProps = () => {
  return (
    <HvProvider cssTheme="scoped" theme={newTheme}>
