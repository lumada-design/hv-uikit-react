...
`GlobalActions` uses the `global` variant by default, rendering an `h1` element and applying sticky positioning.
Use the `section` variant to group related content blocks. Use it sparingly—it introduces strong visual separation, which may not always be necessary.
