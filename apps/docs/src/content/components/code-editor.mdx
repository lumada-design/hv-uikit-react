...
By default, Monaco Editor loads from a CDN. For offline/bundled usage in environments without internet access:
...
To enable offline support, call `configureMonacoOffline()` before using the Code Editor.

**Important**: `configureMonacoOffline()` is a **global configuration** that only needs to be called **once** per application, not per component instance.
...
export default function OfflineCodeEditor() {
  const [isReady, setIsReady] = useState(false);

  useEffect(() => {
    configureMonacoOffline()
      .then(() => setIsReady(true))
      .catch(() => setIsReady(true)); // Continue with CDN fallback
  }, []);

  if (!isReady) {
    return <div>Configuring Monaco Editor for offline use...</div>;
  }
  ...
  "version": "1.0.0",
  "description": "Monaco Editor working offline"
}`}
    />
  );
}
...
#### Option 1: Configure at App Level (Recommended)
...
function App() {
  useEffect(() => {
    configureMonacoOffline().catch(console.warn);
  }, []);
...
#### Option 2: Configure Before First Use
...
export default function MyComponent() {
  ...
  useEffect(() => {
    configureMonacoOffline()
      .then(() => setIsReady(true))
      .catch(() => setIsReady(true)); // CDN fallback
  }, []);
...
The offline configuration provides different levels of support:

- **Full Worker Support**: JSON, CSS, HTML, TypeScript, JavaScript
- **Advanced Plugin Support**: XML (validation, formatting, IntelliSense), SQL (IntelliSense)
- **Basic Support**: All other Monaco-supported languages (syntax highlighting only)

### Behavior

- **CDN by default** - Zero bundle impact without configuration
- **Global configuration** - Configure once, use everywhere
- **Automatic fallback** - Falls back to CDN if offline setup fails
- **Vite optimized** - Designed for Vite bundler (manual config needed for other bundlers)
