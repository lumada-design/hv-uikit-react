...
You can customize the submitted value using the `getSerializedValue` prop, and a custom selected value with the `renderValue` prop.
...
>
  <HvSelect
    ...
    renderValue={(value) => value.map((v) => v.value.toUpperCase()).join(", ")}
    options={[
      { value: "ar", label: "Argentina" },
      { value: "bg", label: "Belgium" },
      { value: "pt", label: "Portugal" },
