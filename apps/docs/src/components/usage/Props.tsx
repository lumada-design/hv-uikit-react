...
const PropsTable2 = ({ title, propsObj }: PropsTableProps) => {
  const makeLink = (name: string) => `${title.toLowerCase()}-${name}`;
      ...
      <HvTableBody>
        {Object.keys(propsObj).length === 0 ? (
          ...
          Object.entries(propsObj).map(([key, propItem]) => (
            <HvTableRow
              key={key}
              id={makeLink(propItem.name)}
              className={classes.row}
            >
              <HvTableCell className="!pl-xs w-[25%]">
                <a
                  href={`#${makeLink(propItem.name)}`}
...
const PropsTable = ({ title, propsObj }: PropsTableProps) => {
  ...
  return (
    <div className="grid" id={id}>
      ...
      <HvTableContainer>
        <PropsTable2 title={title} propsObj={propsObj} />
      </HvTableContainer>
    </div>
  );
};
