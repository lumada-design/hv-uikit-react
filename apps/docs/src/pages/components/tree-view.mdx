...
export const getStaticProps = async ({ params }) => {
  const meta = await getComponentData("TreeView", "core", treeView2Classes, [
    "TreeItem",
  ]);
...
The tree view can be controlled by passing in `expanded`/`onNodeToggle` and `selected`/`onNodeSelect` props to control expansion and selection state respectively.
When using `multiSelect`, the values and callbacks are of type `string[]`, and `string` otherwise.
    ...
    return (
      <HvTreeItem
        ...
        classes={{
          ...
          selected: "border-l-primary bg-primary_20",
        }}
