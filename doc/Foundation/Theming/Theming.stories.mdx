...
Check <LinkTo kind="foundation-provider--main" className="sbdocs-a">the Provider's API documentation</LinkTo> for more details.
...
The function receives a JSON object with the same structure as the definitions found in [here](https://github.com/lumada-design/hv-uikit-react/blob/master/packages/themes/src/dawn.json). **Currently, it must be a complete theme definition,** it will not patch/fill in missing values.
