...
The KPI component is available on the `uikit-react-core` package while all the other visualizations are in the `uikit-react-viz` package. For this reason, all
the information below is only applicable to the visualizations from the `uikit-react-viz` package.

## Provider

Before using any of the UI Kit visualizations, it's necessary to add the `HvVizProvider` component to your application.
This component is responsible for enabling theming capabilities for the visualizations. Without this provider the visualizations will not comply to the
UI Kit themes defined by the [`HvProvider` component](/docs/guides-provider--main) from the core package.
...
- `xAxis`: this visual role represents the chart's horizontal axis. The data will be grouped based on the columns selected for this visual role.
- `measures`: this visual role represents the chart's vertical axis. These are the columns that you want to measure. For each one of the selected columns, you can
  choose an aggregation function to aggregate values with the same `xAxis` value. When no aggregation function is specified, the values will be summed by default.
  The valid aggregation functions are `sum`, `average`, `min`, `max`, and `count`.
- `series`: this visual role can be used to split your data based on specific data table columns.

The `xAxis` and `measures` visual roles are by default of type `categorical` and `continuous`, respectively. These types must be changed if this is not the case
for your data. The valid values are `continuous` for continuous data, `categorical` for discrete data, and `time` for continuous time data.

Please find below an example for the `HvLineChart`.

_Example:_

In this example, a data table with 4 columns (`Country`, `Year`, `Medal`, and `Total`) and 8 rows
was provided. Since we want to showcase the total of medals for each country over the years, the `Year` column was used for the `xAxis` in order to group
...
In this case, no aggregation function was specified for the `measures` since the values will be summed by default. Regarding the `series`,
the `Country` column was used in order to split the data by country and showcase one line for each country.
