...
The `xAxis` and `yAxis` are by default of type `categorical` and `continuous`, respectively. These types must be updated if necessary depending on the data provided.
The valid values are `continuous` for continuous data, `categorical` for discrete data, and `time` for continuous time data.
...
- `groupBy`: this visual role will be used for the chart's horizontal axis. The data will be grouped based on the columns selected for this visual role.
- `measures`: this visual role will be used for the chart's vertical axis. These are the columns that you want to measure. For each one of the selected columns, you can
