...
describe("Banner ", () => {
  const wrapper = mount(
    <HvProvider>
      <BannerWithStyles variant="default" />
    </HvProvider>
  );
  ...
  it("should render the BannerContentWrapper component", () => {
    const bannerComponent = mount(
      <HvProvider>
        <BannerWithStyles variant="default" open />
      </HvProvider>
  ...
  it("should render the icon in the component", () => {
    console.log(variantIcon.error);
    const iconComponent = mount(
      <HvProvider>
        <BannerWithStyles variant="error" open showIcon />
      </HvProvider>
    ).find(Level5Unsuccess16Color);
    console.log(iconComponent.debug());
  ...
  it("shouldn't render the icon in the component", () => {
    const iconComponent = mount(
      <HvProvider>
        <BannerWithStyles variant="error" open showIcon={false} />
      </HvProvider>
    ).find(Level5Unsuccess16Color);
  ...
  it("should render a action on the action container", () => {
    const buttonComponent = mount(
      <HvProvider>
        <BannerWithStyles variant="default" open action={<Button />} />
      </HvProvider>
    ).find(Button);
    expect(buttonComponent.length).toBe(1);
  });
