...
const HeaderSample = withTheme()(({ theme }) => {
  ...
  return (
    <HvHeader position="relative">
      ...
      <HvHeaderActions>
        <HvButton category="icon" onClick={() => console.log("alerts")} aria-label="Open Notifications panel">
          <HvBadge count={1} icon={<Alert boxStyles={boxStyles} />} />
        </HvButton>
        {isMdUp && (
          <HvButton category="icon" onClick={() => console.log("user")} aria-label="Open User panel">
