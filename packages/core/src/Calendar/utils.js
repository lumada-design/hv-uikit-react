...
import { NAV_OPTIONS, REPRESENTATION_VALUES } from "./enums";
...
export const moveDate = (action, yearValue, monthValue, index) => {
  ...
  switch (action) {
    case NAV_OPTIONS.PREVIOUS_MONTH:
      ...
      newMonth = monthCalculation <= 0 ? 12 : monthCalculation;
      break;

    case NAV_OPTIONS.NEXT_MONTH:
      monthCalculation = newMonth + monthJump;
      newYear = monthCalculation >= 12 ? newYear + monthJump : newYear;
      newMonth = monthCalculation >= 12 ? monthJump : monthCalculation;
      break;
