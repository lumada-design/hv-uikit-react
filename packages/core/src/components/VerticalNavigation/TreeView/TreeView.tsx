...
import { TreeViewControlContext, TreeViewStateContext } from "./TreeViewContext";
...
export const HvVerticalNavigationTreeView = forwardRef((props: HvVerticalNavigationTreeViewProps, ref) => {
  const {
    id: idProp,
    className,
    classes,
   ...
   */
  const findOrderInTremauxTree = useCallback((nodeAId: string, nodeBId: string) => {
    if (nodeAId === nodeBId) {
  ...
  const isDivElement = (element): element is HTMLDivElement => {return (element as HTMLDivElement).nodeType === 1}
  ...
  const selectRange = useCallback(
    (event, nodes: { start?, end?, current?: HTMLDivElement | null}, stacked = false) => {
  ...
  const unregisterNode = useCallback((id) => {
    ...
    setFocusedNodeId((oldFocusedNodeId) => {
      if (
        oldFocusedNodeId === id && treeRef.current &&
  ...
  const activeDescendant = focusedNodeId && nodeMap.current[focusedNodeId]
