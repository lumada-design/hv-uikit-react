...
import verticalNavigationHeaderClasses, { HvVerticalNavigationHeaderClasses } from "./headerClasses"
...
export const HvVerticalNavigationHeader = ({
    ...
    return (
        <StyledHeader 
            className={clsx(className, verticalNavigationHeaderClasses.root, classes?.root, 
            !isOpen && clsx(verticalNavigationHeaderClasses.minimized, classes?.minimized))}
            {...others}
            >
            { isOpen && <HvTypography variant="title3">{title}</HvTypography>}
            <HvButton icon variant="secondaryGhost" onClick={onClick} {...buttonProps}>
