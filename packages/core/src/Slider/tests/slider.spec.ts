...
testingThemes.forEach(async (theme) => {
  test.describe(`HvSlider ${theme}`, () => {
    test(`Single slider changes values when interacted`, async ({ page }) => {
      await page.goto(
        `/iframe.html?id=components-inputs-slider--main&args=&viewMode=story${suffix}${theme}`
      );
    ...
    test(`Range slider changes values when interacted`, async ({ page }) => {
      await page.goto(
        `/iframe.html?id=components-inputs-slider--range-slider&args=&viewMode=story${suffix}${theme}`
      );
      await expect(page.locator('[aria-label="Failure Rate-0"]')).toHaveValue("10");
    ...
    test(`Controlled range slider changes values when interacted`, async ({ page }) => {
      await page.goto(
        `http://localhost:9001/iframe.html?id=components-inputs-slider--range-slider-controlled&args=&viewMode=story${suffix}${theme}`
      );
