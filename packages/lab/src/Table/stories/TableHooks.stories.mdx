...
- `useHvTableStyles`: ensures the correct styling of the table by injecting the column's `padding` and `align` options into the `HvTableHeader` and `HvTableCell` components (via `getHeaderProps` and `getCellProps`, respectively).
...
_Note: This hook depends on the `useRowSelect` hook from `react-table`, but when using `useHvTable` only `useHvRowSelecion` needs to be installed._
