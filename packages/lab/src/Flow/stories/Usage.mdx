...
const nodeGroups = {
  ...
  models: {
    ...
    icon: <Favorite />,
    items: {
      prediction: [ type: "model" , label: "ML Model Prediction" ]
    }
  },
}
```
...
If you have defined `nodeGroups`, there are a few properties that should be set on your node to simplify its customization, namely `group` and `groupItem`.
By providing the group id and group item id of the node, the `color`, `icon`, `title`, and `subtitile` properties will be automatically set
using the values defined by the node group. However, if `group` and `groupItem` are not provided, you can set `color`, `icon`, `title`, and `subtitile`
