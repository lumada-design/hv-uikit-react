...
const ruleToMongo = (rule, timezone) => {
  ...
  if (
    (rule.value == null || (typeof rule.value === "string" && rule.value.trim() === "")) &&
    rule.operator !== "Empty" &&
  ...
  switch (rule.operator) {
    ...
    case "range":
      if (
        ...
        value = {
          $gte: formatToUTC(`${rule.value.start.date} ${rule.value.start.time}`, timezone),
          $lte: formatToUTC(`${rule.value.end.date} ${rule.value.end.time}`, timezone),
        };
      } else if (
        rule.value != null &&
